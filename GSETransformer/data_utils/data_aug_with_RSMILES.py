...
def preprocess(save_dir, reactants, products,set_name, augmentation=1, reaction_types=None,root_aligned=True,character=False, processes=-1):
    ...
    if data[0]['root_aligned']:
        print(f'doing root_aligned for {set_name}')
    else:
        print(f'doing canonical for {set_name}')
...
def multi_process(data):
    ...
    else:# extract canonical reaction for 'unqualified status reaction'
        pro_atom_map_numbers = list(map(int, re.findall(r"(?<=:)\d+", product)))
        reactant = reactant.split(".")
        cano_product = clear_map_canonical_smiles(product)
        cano_reactanct = ".".join([clear_map_canonical_smiles(rea) for rea in reactant if
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dataset',type=str,default='biochem_npl',
                        help='choose in [biochem_npl, biochem_npl_clean, uspto-50k]')
    ...
    print(args)
    # -dataset biochem_npl -augmentation 20
    # -dataset biochem_npl -augmentation 20  -canonical
    ...
    datadir = f'{os.path.dirname(os.path.dirname(__file__))}/data/{args.dataset}'

    if args.canonical:
        postfix = 'Cano'
    else:
        postfix = 'RSMILES'
    savedir = f'{os.path.dirname(os.path.dirname(__file__))}/data/{args.dataset}_{args.augmentation}xaug_{postfix}'
    if not os.path.exists(savedir):
        os.makedirs(savedir)
    #  read files from datadir and save fiels to savedir
    for i, data_set in enumerate(datasets):
        ...
        if data_set in ['train', 'val']:  # preform augment for ['train', 'val'] # you can change it according to your need
            src_data, tgt_data = preprocess(
                ...
                args.augmentation,
                reaction_types=None,
                root_aligned=not args.canonical, # cano or Root_aline
                character=args.character,
                processes=args.processes,
            )
        else:       # no aug for  test
            aug_time = 1
            src_data, tgt_data = preprocess(
                savedir,
                sub_react_list,
                ...
                aug_time,
                reaction_types=None,
                root_aligned=args.canonical, #do cano or Root_aline, both ok, just no need to aug
                character=args.character,
                processes=args.processes,
            )
