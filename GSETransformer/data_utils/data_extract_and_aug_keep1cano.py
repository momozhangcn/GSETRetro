...
def smi_tokenizer(smi):
    pattern = "(\[[^\]]+]|Br?|Cl?|Mg?|Fe?|Se?|Mn?|Zn?|Cu?|Na?|Al?|N|O|S|P|F|I|b|c|n|o|s|p|\(|\)|\.|=|#|-|\+|\\\\|\/|:|~|@|\?|>|\*|\$|\%[0-9]{2}|[0-9])"
...
def multi_process(data):
    ...
    cano_product_token = smi_tokenizer(cano_product)
    cano_reactanct_token = smi_tokenizer(cano_reactanct)

    return_status['src_data'].append(cano_product_token)
    return_status['tgt_data'].append(cano_reactanct_token)
    ...
    one_side_aug_time = augmentation//2
    for _ in range(one_side_aug_time):
...
if __name__ == '__main__':
    ...
    parser.add_argument("-aug_time", type=int, default=20)
    ...
    savedir = f'./dataset/{args.dataset}_{args.aug_time}xaug_keep1'
