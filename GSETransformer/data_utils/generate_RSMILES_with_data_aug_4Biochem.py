...
if __name__ == '__main__':
    ...
    datadir = f'/data/zhangmeng/AAA_GSETRetro/aMy-ONMT/data/{args.dataset}'
    if args.canonical:
        postfix = 'Cano'
    else:
        postfix = 'RSmiles'
    savedir = f'/data/zhangmeng/AAA_GSETRetro/aMy-ONMT/data/final_{args.dataset}_{args.augmentation}xaug_{postfix}'

    if not os.path.exists(savedir):
        os.makedirs(savedir)
    for i, data_set in enumerate(datasets):
        # with open(os.path.join(datadir, f"src-{data_set}.txt"), "r") as f_src,\
        #         open(os.path.join(datadir, f"tgt-{data_set}.txt"), "r") as f_tgt:
        # src_list = f_src.readlines()
        # tgt_list = f_tgt.readlines()
