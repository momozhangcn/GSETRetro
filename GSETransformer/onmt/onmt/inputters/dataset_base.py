...
class Dataset(TorchtextDataset):
    ...
    def __init__(self, fields, readers, data, dirs, sort_key,
        ...
        self.src_vocabs = []
        examples = []
        # for k, v in fields.items():   print(k, v) # check # check
        # # src <onmt.inputters.text_dataset.TextMultiField  #tgt <onmt.inputters.text_dataset.TextMultiField
        # # indices <torchtext.data.field.Field #graph <torchtext.data.field.Field
        for ex_dict in starmap(_join_dicts, zip(*read_iters)):
            if can_copy:# check ，do not get here
                src_field = fields['src']
                tgt_field = fields['tgt']
                edge_index_field = fields['edge_index']
                # this assumes src_field and tgt_field are both text
                src_ex_vocab, ex_dict = _dynamic_dict(
                    ex_dict, src_field.base_field, tgt_field.base_field)
                self.src_vocabs.append(src_ex_vocab)

            #ex_dict['graph'] = myutils.str2edge_index(ex_dict['src']) #add
