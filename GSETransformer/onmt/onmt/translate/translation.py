...
class TranslationBuilder(object):
    ...
    def from_batch(self, translation_batch):
        ...
        for b in range(batch_size):
            ...
            if 0 < len(preds[b]) < self.n_best:
                index_lst = []
                index_lst_ok = False
                while not index_lst_ok:
                    index_lst += list(range(0, len(preds[b]), 1))
                    if len(index_lst) >= self.n_best:
                        index_lst_ok = True
                pred_sents = [self._build_target_tokens(
                        src[:, b] if src is not None else None,
                        src_vocab, src_raw,
                        preds[b][n], attn[b][n]) for n in index_lst[:50]]
            # elif len(preds[b]) == 0:
            #     print(f'got here len(preds[b]) {len(preds[b])}, n_best {self.n_best}')
            #     pred_sents = [self._build_target_tokens(
            #         src[:, b] if src is not None else None,
            #         src_vocab, src_raw,
            #         src[:, b], None) for n in range(self.n_best)]
            else:
                pred_sents = [self._build_target_tokens(
                    src[:, b] if src is not None else None,
                    src_vocab, src_raw,
