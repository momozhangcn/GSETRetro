...
class Translator(object):
    ...
    def __init__(
        ...
        self.edge_index_reader = edge_index_reader ## add it here # check change for translator
    ...
    def from_opt(
        ...
        return cls(
            ...
            logger=logger,
            seed=opt.seed,
            edge_index_reader=edge_index_reader,
            multi_step=opt.multi_step)
    ...
    def translate(
        ...
        data = inputters.Dataset(
            self.fields,
            readers=([self.src_reader, self.tgt_reader, self.edge_index_reader]
                     if tgt else [self.src_reader, self.edge_index_reader]),
            data=[("src", src), ("tgt", tgt), ("edge_index", edge_index)] if tgt else [("src", src), ("edge_index", edge_index)],
        ...
        if not self.multi_step:         # not multi_step, single step, print tqdm
        ...
        for batch in iterator:
            ...
            for trans in translations:
                #print([trans.pred_scores[:self.n_best]])
        ...
        if self.report_score and not self.multi_step:# if multi_step,do not print
    ...
    def _run_encoder(self, batch):
        ...
        src_input = (src, batch.edge_index)
        enc_states, memory_bank, src_lengths = self.model.encoder(
            src_input, src_lengths)
    ...
    def _translate_batch(
        ...
        # get whole output
        # print('results["scores"]     ', len(results["scores"]), results["scores"] )
        # print('results["predictions"]', len(results["predictions"]),results["predictions"] )
        # print('results["attention"]  ', len(results["attention"]),results["attention"] )
        # get single output
        # print(results["scores"][0])
        # print(results["predictions"][0])
        # print(results["attention"][0])
