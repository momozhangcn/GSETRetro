...
class EarlyStopping(object):
    ...
    def __init__(self, tolerance, scorers=DEFAULT_SCORERS):
        ...
        self.current_step_best_ppl = 0   #add
        self.current_step_best_acc = 0   #add
    ...
    def __call__(self, valid_stats, step):
        ...
        # just_ppl_improved
        elif self.early_stopping_scorers[0].is_improving(valid_stats):
            self._update_ppl_increasing(valid_stats, step)
        # just_acc_improved
        elif self.early_stopping_scorers[1].is_improving(valid_stats):
            self._update_acc_increasing(valid_stats, step)
        # decreased
    ...
    def _update_increasing(self, valid_stats, step):
        ...
        self.current_step_best_ppl = step  # add
        self.current_step_best_acc = step  # add
    ...
    def _update_ppl_increasing(self, valid_stats, step):
        self.current_step_best_ppl = step
        scorer = self.early_stopping_scorers[0]
        logger.info(
            "Model is improving {}: {:g} --> {:g}.".format(
                scorer.name, scorer.best_score, scorer(valid_stats))
        )
    ...
    def _update_acc_increasing(self, valid_stats, step):
        self.current_step_best_acc = step
        scorer = self.early_stopping_scorers[1]
        logger.info(
            "Model is improving {}: {:g} --> {:g}.".format(
                scorer.name, scorer.best_score, scorer(valid_stats))
        )
